function estimate = ExtendedKalmanFilter_series(prior_est, accSampled,gyrSampled,magSampled,magnetic, debug_flag)
...
for index = 1:1:height(accSampled)
    ...
    y_hat = [ quat2rotmat(prior_est)' * mag_field];
...
%     final_q = final_q * sign(final_q(1));
    ...
    S = H*final_P*H' + R;
    K = (final_P*H') / S;
    post_P = final_P - K*S*K';
    post_est = final_q + K* error;
